...
def Filtrar():
    ...
    ConsultaRecF = "SELECT recuperandas, cnpj, exposicao, dataagc, rjextinta, iddados_rj FROM dados_rj ORDER by dataagc"
    cursor.execute(ConsultaRecF)
...
def DeletarAJ():
    global id_aj_var
    confirmadeletaraj = messagebox.askyesno("CONFIRMA A EXCLUSÃO", "Você deseja excluir o AJ selecionado?")
    if confirmadeletaraj:
        dados_aj_selecionados = relaj.focus()
        aj_selecionado = relaj.item(dados_aj_selecionados)
        id_aj = aj_selecionado['values'][3]
        id_aj_var = id_aj
        comandodeletar = f'DELETE FROM admjud WHERE iddados_rj = "{id_aj_var}"'
        cursor.execute(comandodeletar)
        conexao.commit()
        tkinter.messagebox.showinfo(title="AVISO", message="Dados do ADMINISTRADOR JUDICIAL deletados")
        ConsultarAj()
    else:
        return
...
def DeletarREC():
    global id_rec_var
    confirmadeletarrec = messagebox.askyesno("CONFIRMA A EXCLUSÃO", "Você deseja excluir a recuperanda selecionado?")
    if confirmadeletarrec:
        dados_rec_selecionados = relrec.focus()
        rec_selecionada = relrec.item(dados_rec_selecionados)
        id_rec = rec_selecionada['values'][5]
        id_rec_var = id_rec
        comandodeletarrec = f'DELETE FROM dados_rj WHERE iddados_rj = "{id_rec_var}"'
        cursor.execute(comandodeletarrec)
        conexao.commit()
        tkinter.messagebox.showinfo(title="AVISO", message="Dados da RECUPERANDA deletados")
        ConsultarRec()
    else:
        return
...
def DadosAj():
    ...
    dados_aj_selecionados = relaj.focus()
    aj_selecionado = relaj.item(dados_aj_selecionados)
    nome_aj = aj_selecionado['values'][0]
    nome_site = aj_selecionado['values'][1]
    nome_contatoaj = aj_selecionado['values'][2]
    id_aj = aj_selecionado['values'][3]
    nomeaj.insert(0, nome_aj)
    siteaj.insert(0, nome_site)
    contatoaj.insert(0, nome_contatoaj)
    id_aj_var = id_aj
...
def ConsultarAj():
    ...
    ConsultaAj = f'SELECT administradorjudicial, siteadm, contatoadm, iddados_rj FROM admjud'
...
def AlterarAJ():
    ...
    AlteraAjBD = f'UPDATE admjud SET administradorjudicial = "{administradorjudicial}", siteadm = "{siteadm}", contatoadm = "{contatoadm}" WHERE iddados_rj = {id_aj_var}'
    cursor.execute(AlteraAjBD)
    conexao.commit()
    tkinter.messagebox.showinfo(title="ALTERAÇÃO CONFIRMADA", message="Alteração de AJ Realizada com Sucesso")
    nomeaj.delete(0, tk.END)
    siteaj.delete(0, tk.END)
    contatoaj.delete(0, tk.END)
    ConsultarAj()
...
def VerificarAJ():
    ...
    if administradorjudicial is None or administradorjudicial == '' or siteadm is None or siteadm == '' or contatoadm is None or contatoadm == '':
        tkinter.messagebox.showinfo(title="AVISO", message="Todos os campos devem ser preenchidos.")
        return
    sejaexisteaj = f'SELECT EXISTS (SELECT * FROM admjud WHERE administradorjudicial = "{administradorjudicial}")'
    cursor.execute(sejaexisteaj)
    resultadoaj = cursor.fetchone()
    sejaexisteaj2 = f'SELECT EXISTS (SELECT * FROM admjud WHERE siteadm = "{siteadm}")'
    cursor.execute(sejaexisteaj2)
    resultadoaj2 = cursor.fetchone()
    sejaexisteaj3 = f'SELECT EXISTS (SELECT * FROM admjud WHERE contatoadm = "{contatoadm}")'
    cursor.execute(sejaexisteaj3)
    resultadoaj3 = cursor.fetchone()
    if resultadoaj[0] or resultadoaj2[0] or resultadoaj3[0]:
        AlterarAJ()
    else:
        CADASTRARAdmJud()
...
relaj = ttk.Treeview(columns= ("coluna 1", "coluna 2", "coluna 3", "coluna 4"))
...
contatoaj = tk.Entry()
botaoincaj = tk.Button(text= "CONFIRMAR INCLUSÃO/ALTERAÇÃO DO AJ", command=VerificarAJ)
...
relaj.column("coluna 4", width= 0, stretch = "NO")
...
botaoteste = tk.Button(TelaInicial, text = "ALTERAR REC", command= DadosRec)
botaoteste.grid(row = 12, column = 8, columnspan = 2)
botaotesteAj = tk.Button(TelaInicial, text = "ALTERAR AJ", command= DadosAj)
botaotesteAj.grid(row = 12, column = 19, columnspan = 2)
botaodeletarrec = tk.Button(TelaInicial, text = "DELETAR REC", command= DeletarREC)
botaodeletarrec.grid(row = 12, column = 5, columnspan = 2)
botaodeletaraj = tk.Button(TelaInicial, text = "DELETAR AJ", command= DeletarAJ)
botaodeletaraj.grid(row = 12, column = 16, columnspan = 2)
botaofiltradataagc = tk.Button(TelaInicial, text= "FILTRAR POR DATA DE AGC", command= Filtrar)
botaofiltradataagc.grid(row = 4, column = 9, columnspan = 2)
